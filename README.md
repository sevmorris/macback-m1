...
Some of my dotfiles are live in ~/dotfiles and Github. Locally they're symlinked in ~/.
The .aliases file has an alias (`backup`) for backing up user preferences and some dotfiles (excluding the ones in ~/dotfiles. It rsyncs to a Dropbox folder, which you would need to create before running. Refer to the alias for dir names.
